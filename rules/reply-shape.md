...
- Draw one item under the branch being worked, the marked one or the first one, and count the rest - even when every one of them is waiting on you. Mark at most one row, and none in a reply that opens nothing. Keep a row to one line, wrapping by hand at about sixty characters. {queue-tree=always-on|on-switch-only}
