...
- Draw the queue as a list or as a tree, never both in one reply. The list holds {queue-limit}, then `...N more pending` when any are hidden, N being every item below them. Write no such line when every item is shown. {queue-tree=off|on-switch-only} {queue-limit=set}
