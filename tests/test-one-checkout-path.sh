...
cloned="$(sed -n 's/^[[:space:]]*mv [^ ]*\.new \(\/opt\/[A-Za-z0-9._-]*\)$/\1/p' \
  "$REPOSITORY/clients/claude-code-cloud/install.md" | head -1)"
...
grep --quiet --fixed-strings "$cloned/distributions/claude-code-cloud/install.sh" "$REPOSITORY/clients/claude-code-cloud/install.md"
