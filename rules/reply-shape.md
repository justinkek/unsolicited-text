...
- A reply draws one shape and never both. The list holds every item. {queue-tree=off|on-switch-only} {queue-limit=unset}
- A reply draws one shape and never both. The list holds the first {queue-limit} items, then `...N more pending`, with N the number left unshown and anything raised this turn below that line. {queue-tree=off|on-switch-only} {queue-limit=set}
