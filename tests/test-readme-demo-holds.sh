...
printf "Test group: the recorded replies are what the hook says they are\n"

before="$(note_for "$DEMO/before.txt")"
[ -n "$before" ]
assert "the reply recorded without the plugin is over the ceiling" "$?" "the hook recorded nothing"

after="$(note_for "$DEMO/after.txt")"
[ -z "$after" ]
...
kept="$(awk '
  ...
  NF { kept++ }
  END { print kept + 0 }' "$DEMO/after.txt")"
...
printf "\nTest group: the recordings are not older than what they record\n"
