...
| how each install method works | `installs/<distribution>.md`, and `updates/` and `uninstalls/` beside it. Each builds that distribution's README; `INSTALL.md` is the table pointing at them, and `UPDATE.md` and `UNINSTALL.md` carry every section |
