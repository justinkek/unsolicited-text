...
Last verified at 0.3.12, in the desktop app and in the phone app: a new
conversation carried the rules, the reminder arrived with every message, and a
ten-line reply came back as a note naming the ceiling of eight. The phone run
reported its home directory inside a cloud container, so a session driven from
a phone runs where every other Cowork session runs.
